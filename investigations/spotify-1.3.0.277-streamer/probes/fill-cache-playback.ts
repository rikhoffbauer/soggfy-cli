...
const original=await pl.getState(), beforeCached=await isCached();if(original.isPaused)await pl.resume();await new Promise(r=>setTimeout(r,500));const beforeState=await pl.getState(),beforeSampledAt=Date.now();
...
const afterCached=await isCached(),afterState=await pl.getState(),afterSampledAt=Date.now();if(original.isPaused){await pl.pause();await pl.seekTo(original.positionAsOfTimestamp);await new Promise(r=>setTimeout(r,150))}const restored=await pl.getState();
const files=await pb.getFiles(TRACK);const match=files.find(x=>x.fileId===FILE);
return JSON.stringify({beforeCached,afterCached,total,summary:{events:events.length,data:events.reduce((a,x)=>a+x.dataLength,0),previouslyCached:events.reduce((a,x)=>a+x.previouslyCached,0),fromNetwork:events.reduce((a,x)=>a+x.fromNetwork,0),firstAtMs:events[0]?.atMs,lastAtMs:events.at(-1)?.atMs},storage:match,before:{sampledAt:beforeSampledAt,uri:beforeState?.item?.uri,isPaused:beforeState?.isPaused,position:beforeState?.positionAsOfTimestamp,timestamp:beforeState?.timestamp},after:{sampledAt:afterSampledAt,uri:afterState?.item?.uri,isPaused:afterState?.isPaused,position:afterState?.positionAsOfTimestamp,timestamp:afterState?.timestamp},effectiveAdvance:(afterState?.positionAsOfTimestamp+(afterState?.isPaused?0:afterSampledAt-afterState?.timestamp))-(beforeState?.positionAsOfTimestamp+(beforeState?.isPaused?0:beforeSampledAt-beforeState?.timestamp)),wallAdvance:afterSampledAt-beforeSampledAt,restored:{uri:restored?.item?.uri,isPaused:restored?.isPaused,position:restored?.positionAsOfTimestamp}});
})()`;
