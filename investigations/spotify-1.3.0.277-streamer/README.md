...
The normal daemon uses its configured supported Spotify build; do not assume it is the same version as this isolated investigation. During the 2026-09-18 closure run, direct daemon-child ownership inspection verified the normal runtime as Spotify `1.2.99.317` at `~/.soggfy/workspace/PatchedSpotify.app`, executable SHA-256 `42add29ff06934cd612984ddce079705165296160a88b4ffa2ba604c18206d8b`. Treat that as recorded evidence for that run, not a permanent configured-version constant.
...
See `RESULTS.md` for the production prefetch decision-gate summary and
`NATIVE-SOURCE-RESULTS.md` for the closed independent-native-source feasibility
investigation.

## Semantic supervision

The spec/plan also has a derived, non-authoritative TypeSafe supervision contract at:

`docs/superpowers/supervision/2026-09-18-single-instance-prefetch.json`

The current evidence ledger is `supervision-state.json`. Run exact gates without an API call:

```sh
bun run supervise -- gates
```

Run semantic audits with named question sets and hashed source context, for example:

```sh
bun run supervise -- audit \
  --set result_interpretation \
  --context phase2_result=investigations/spotify-1.3.0.277-streamer/results/phase2-live-probes-20260918.json
```

See `docs/superpowers/supervision/README.md` for the full harness contract and usage discipline.
