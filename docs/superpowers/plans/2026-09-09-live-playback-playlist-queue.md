...
- [x] **Step 1: Write failing normalization and pagination tests**
...
Also fake two Pathfinder responses and assert `fetchAllSpotifyPlaylistTracks()` requests offsets `0` then `2`, returns source order, and reports unavailable/non-track entries rather than silently dropping them.- [x] **Step 2: Run the new test and confirm it fails**

Run: `bun test test/spotify-playlist.test.ts`

Expected: FAIL because `src/core/spotify-playlist.ts` does not exist.

- [x] **Step 3: Implement the playlist client**
...
- [x] **Step 4: Run playlist-client tests**
...
Add a chained interruption case proving `C -> B -> A -> normal` ordering when each interrupted active entry is inserted directly behind the current priority head.- [x] **Step 2: Run queue tests and confirm failure**
...
- [x] **Step 3: Implement the focused queue abstraction**
...
- [x] **Step 4: Run queue and job-registry tests**
...
- [x] **Step 1: Add failing URL and source-contract tests**
...
Expected: FAIL on missing priority-play/API behavior.- [x] **Step 3: Integrate priority interruption into `SpotifyPoolManager`**
...
- [x] **Step 4: Add explicit playlist routes without side effects**
...
- [x] **Step 5: Run server tests and typecheck**
...
- [x] **Step 1: Write failing delayed-file and append tests**
...
Expected: FAIL because `streamGrowingFile` does not exist.- [x] **Step 3: Implement growing-file streaming**
...
- [x] **Step 5: Run streaming and existing media tests**
...
- [x] **Step 3: Implement explicit search actions and playlist detail state**
...
- [x] **Step 4: Wire Play, Queue, Queue all, and additional pages**
...
- [x] **Step 5: Run frontend model test and typecheck**
...
- [x] **Step 1: Write failing player-state tests**
...
- [x] **Step 2: Run the player-model test and confirm failure**

Run: `cd webapp && bun test src/components/soggfy/__tests__/player-model.test.ts`

Expected: FAIL because the player helpers do not exist.- [x] **Step 3: Let PlayerBar attach before completion**
...
- [x] **Step 4: Run player and workspace tests plus typecheck**
...
- [x] **Step 2: Restart and probe the daemon API**
...
- [x] **Step 3: Verify live audio begins before completion**
...
- [x] **Step 4: Render desktop and mobile UI with existing Puppeteer Core**
...
- [x] **Step 5: Fix only observed regressions and rerun the relevant failing test first**
