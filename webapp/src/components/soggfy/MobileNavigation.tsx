...
import { IconActivityHeartbeat, IconDownload, IconHeart, IconHistory, IconSearch } from "@tabler/icons-react";
...
const PAGES: Array<{ page: WorkspacePage; label: string; icon: ReactNode }> = [
  ...
  { page: "spotify", label: "Spotify", icon: <IconHeart /> },
...
export function MobileNavigation({ activePage, onNavigate }: { activePage: WorkspacePage; onNavigate: (page: WorkspacePage) => void }) {
  return (
    <nav className="grid h-11 shrink-0 grid-cols-5 border-b border-white/[0.06] bg-[#0b0d10] lg:hidden" aria-label="Primary">
      {PAGES.map((item) => (
