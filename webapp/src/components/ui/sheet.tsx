import * as React from "react"
import { Dialog as SheetPrimitive } from "@base-ui/react/dialog"

import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import { IconX } from "@tabler/icons-react"

/**
 * Sheet component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function Sheet({ ...props }: SheetPrimitive.Root.Props) {
  return <SheetPrimitive.Root data-slot="sheet" {...props} />
}

/**
 * SheetTrigger component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetTrigger({ ...props }: SheetPrimitive.Trigger.Props) {
  return <SheetPrimitive.Trigger data-slot="sheet-trigger" {...props} />
}

/**
 * SheetClose component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetClose({ ...props }: SheetPrimitive.Close.Props) {
  return <SheetPrimitive.Close data-slot="sheet-close" {...props} />
}

/**
 * SheetPortal component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetPortal({ ...props }: SheetPrimitive.Portal.Props) {
  return <SheetPrimitive.Portal data-slot="sheet-portal" {...props} />
}

/**
 * SheetOverlay component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
  return (
    <SheetPrimitive.Backdrop
      data-slot="sheet-overlay"
      className={cn("data-open:animate-in data-closed:animate-out data-closed:fade-out-0 data-open:fade-in-0 bg-black/10 duration-100 data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs fixed inset-0 z-50", className)}
      {...props}
    />
  )
}

/**
 * SheetContent component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetContent({
  className,
  children,
  side = "right",
  showCloseButton = true,
  ...props
}: SheetPrimitive.Popup.Props & {
  side?: "top" | "right" | "bottom" | "left"
  showCloseButton?: boolean
}) {
  return (
    <SheetPortal>
      <SheetOverlay />
      <SheetPrimitive.Popup
        data-slot="sheet-content"
        data-side={side}
        className={cn("bg-background data-open:animate-in data-closed:animate-out data-[side=right]:data-closed:slide-out-to-right-10 data-[side=right]:data-open:slide-in-from-right-10 data-[side=left]:data-closed:slide-out-to-left-10 data-[side=left]:data-open:slide-in-from-left-10 data-[side=top]:data-closed:slide-out-to-top-10 data-[side=top]:data-open:slide-in-from-top-10 data-closed:fade-out-0 data-open:fade-in-0 data-[side=bottom]:data-closed:slide-out-to-bottom-10 data-[side=bottom]:data-open:slide-in-from-bottom-10 fixed z-50 flex flex-col gap-4 bg-clip-padding text-sm shadow-lg transition duration-200 ease-in-out data-[side=bottom]:inset-x-0 data-[side=bottom]:bottom-0 data-[side=bottom]:h-auto data-[side=bottom]:border-t data-[side=left]:inset-y-0 data-[side=left]:left-0 data-[side=left]:h-full data-[side=left]:w-3/4 data-[side=left]:border-r data-[side=right]:inset-y-0 data-[side=right]:right-0 data-[side=right]:h-full data-[side=right]:w-3/4 data-[side=right]:border-l data-[side=top]:inset-x-0 data-[side=top]:top-0 data-[side=top]:h-auto data-[side=top]:border-b data-[side=left]:sm:max-w-sm data-[side=right]:sm:max-w-sm", className)}
        {...props}
      >
        {children}
        {showCloseButton && (
          <SheetPrimitive.Close
            data-slot="sheet-close"
            render={
              <Button
                variant="ghost"
                className="absolute top-3 right-3"
                size="icon-sm"
              />
            }
          >
            <IconX
            />
            <span className="sr-only">Close</span>
          </SheetPrimitive.Close>
        )}
      </SheetPrimitive.Popup>
    </SheetPortal>
  )
}

/**
 * SheetHeader component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="sheet-header"
      className={cn("gap-0.5 p-4 flex flex-col", className)}
      {...props}
    />
  )
}

/**
 * SheetFooter component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="sheet-footer"
      className={cn("gap-2 p-4 mt-auto flex flex-col", className)}
      {...props}
    />
  )
}

/**
 * SheetTitle component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetTitle({ className, ...props }: SheetPrimitive.Title.Props) {
  return (
    <SheetPrimitive.Title
      data-slot="sheet-title"
      className={cn("text-foreground text-base font-medium", className)}
      {...props}
    />
  )
}

/**
 * SheetDescription component styled for this UI kit.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
function SheetDescription({
  className,
  ...props
}: SheetPrimitive.Description.Props) {
  return (
    <SheetPrimitive.Description
      data-slot="sheet-description"
      className={cn("text-muted-foreground text-sm", className)}
      {...props}
    />
  )
}
/**
 * Props for Sheet.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetProps = React.ComponentProps<typeof Sheet>

/**
 * Props for SheetTrigger.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetTriggerProps = React.ComponentProps<typeof SheetTrigger>

/**
 * Props for SheetClose.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetCloseProps = React.ComponentProps<typeof SheetClose>

/**
 * Props for SheetPortal.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetPortalProps = React.ComponentProps<typeof SheetPortal>

/**
 * Props for SheetOverlay.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetOverlayProps = React.ComponentProps<typeof SheetOverlay>

/**
 * Props for SheetContent.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetContentProps = React.ComponentProps<typeof SheetContent>

/**
 * Props for SheetHeader.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetHeaderProps = React.ComponentProps<typeof SheetHeader>

/**
 * Props for SheetFooter.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetFooterProps = React.ComponentProps<typeof SheetFooter>

/**
 * Props for SheetTitle.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetTitleProps = React.ComponentProps<typeof SheetTitle>

/**
 * Props for SheetDescription.
 *
 * @example
 * ```tsx
 * <Sheet>
 *       <SheetTrigger render={<Button />}>Open sheet</SheetTrigger>
 *       <SheetContent>
 *         <SheetHeader>
 *           <SheetTitle>Team settings</SheetTitle>
 *           <SheetDescription>Update team permissions and access.</SheetDescription>
 *         </SheetHeader>
 *         <SheetFooter>
 *           <Button>Save changes</Button>
 *         </SheetFooter>
 *       </SheetContent>
 *     </Sheet>
 * ```
 */
export type SheetDescriptionProps = React.ComponentProps<typeof SheetDescription>


export {
  Sheet,
  SheetTrigger,
  SheetClose,
  SheetContent,
  SheetHeader,
  SheetFooter,
  SheetTitle,
  SheetDescription,
}
